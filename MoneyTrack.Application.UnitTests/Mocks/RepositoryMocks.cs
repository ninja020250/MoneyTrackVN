...
namespace MoneyTrack.Application.UnitTests.Mocks;

public class RepositoryMocks
{
    public static Mock<ITransactionRepository> GetTransactionRepository()
    {
        Guid userId = new Guid("b34ac404-2285-4af1-88f6-1e4692d6df45");
        var categoryId = Guid.Parse("88d9d4e4-8a6d-42b9-9495-74bfe29c1b97"); // Food category ID
        var shoppingCategoryId = Guid.Parse("b3e7df94-22d7-4f8e-9f1d-9f8eb6dfc1c2"); // Shopping category ID
        var transactions = new List<TransactionEntity>
        {
            ...
            new TransactionEntity
            {
                Id = Guid.NewGuid(), Amount = 50.0,
                Description = "Transport", CategoryId = Guid.NewGuid(),
                UserId = userId,
                ExpenseDate = DateTime.Now.AddDays(-1)
            },
            new TransactionEntity
            {
                Id = Guid.NewGuid(), Amount = 200.0,
                Description = "Rent",
        ...
        mockRepo.Setup(r => r.GetListByUserIdAsync(userId)).ReturnsAsync(transactions);
        mockRepo.Setup(r => r.IsTransactionExist(It.IsAny<Guid>()))
            .ReturnsAsync((Guid id) => transactions.Any(t => t.Id == id));

        // Setup for Add operations
        mockRepo.Setup(r => r.AddAsync(It.IsAny<TransactionEntity>()))
            .ReturnsAsync((TransactionEntity transaction) =>
            {
        ...
        // Setup for Update operations
        mockRepo.Setup(r => r.UpdateAsync(It.IsAny<TransactionEntity>()))
            .Returns(Task.CompletedTask)
            .Callback<TransactionEntity>(transaction => { transaction.LastModifiedDate = DateTime.Now; });

        // Setup for Delete operations
        mockRepo.Setup(r => r.DeleteAsync(It.IsAny<TransactionEntity>()))
            .Returns(Task.CompletedTask);

        // Setup for GetByIdAsync - returns null for non-existent transactions
    ...
    public static Mock<ITransactionCategoryRepository> GetTransactionCategoryRepository()
    {
        var categories = new List<TransactionCategoryEntity>
        {
        ...
        var mockRepo = new Mock<ITransactionCategoryRepository>();
        mockRepo.Setup(r => r.IsCategoryCodeExist(It.IsAny<string>()))
            .ReturnsAsync((string code) => categories.Any(c => c.Code == code));

        mockRepo.Setup(r => r.GetByCodeAsync(It.IsAny<string>()))
            .ReturnsAsync((string code) => categories.FirstOrDefault(c => c.Code == code));
